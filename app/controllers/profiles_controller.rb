...
class ProfilesController < ApplicationController
  ...
  def update
    if current_user.update_attributes(user_params)
      redirect_to profile_path, success: '更新个人资料成功！'
    else
  ...
  def crop_avatar
    redirect_to profile_path, warnning: '您还没有上传头像！' unless current_user.avatar.url.present?
  end
  
  def update_avatar
    if params[:user].present? && current_user.update_attributes(user_params)
      if params[:cropping_avatar]
        redirect_to profile_path, success: '头像更新成功！'
      else
        redirect_to crop_avatar_path, info: '如果您对头像位置不太满意，您还可以进一步请选择头像区域！'
      end
    else
  ...
  def update_password
    if current_user.update_password(user_params)
      redirect_to profile_path, success: '更新密码成功！'
    else
      flash.now[:danger] = '密码更新时有错误发生！'
